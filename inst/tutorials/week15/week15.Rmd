...
Welcome to the last tutorial of the Quantitative Methods in International Relations course! Today we will replicate some of the results of Sarah Hobolt\'s (2016) paper: \"The Brexit vote: a divided nation, a divided continent\". As always, we will start with some knowledge questions and then continue with some coding-exercises. Have fun!

## Knowledge Questions

```{r q1, echo = FALSE}
question("Why is it useful to rely on logstic regression models for our referendum vote variable?",
...
question("Why do we need a link function for logistic regression analyses?",
  ...
  answer("Because logistic regression models analyse the responses of people participating in surveys.", message = "They do sometimes, but that does not affect the need for a link-function."),
...
Calculate the predicted probability of voting `Leave` for individuals with the highest value for `EuropeanIdentity`, holding all other variables at their means or medians. Remember, that the variables (apart from the `age`-variable) are questionnaire variables. Though they are numeric, the answer options have been categorical. What is the predicted probability to vote `Leave` for such voters? Round to two decimals.
...
Now, calculate predicted probabilities, this time for the lowest value of `EuropeanIdentity`, holding everything else at its means or medians. What is the predicted probability for those individuals? Again, round to two decimals.
...
learnrhash::encoder_ui(default_ui(url =
  "https://ilias.uni-mannheim.de/ilias.php?baseClass=ilObjSurveyGUI&ref_id=1114229&cmd=infoScreen"
))
```