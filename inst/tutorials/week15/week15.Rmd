...
question_text("  What is the predicted probability to vote `Leave` for such voters? Round to two decimals.",
  ...
  allow_retry = TRUE,
  incorrect = "Hint: Start with the lab code and extend the scenarios to the new variables we included. In case you received a prediction of 0.049/0.05, make sure that you set the qurstionnaire items to their medians."
)
```
...
question_text("What is the predicted probabiliy for those individuals? Again, round to two decimals.",
  ...
  allow_retry = TRUE,
  incorrect = "Hint: Same as before, only the value for `EuropeanIdentity` has to be changed. In case you received a prediction of 0.86, make sure that you set the qurstionnaire items to their medians."
)
```
